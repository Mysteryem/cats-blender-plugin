...
dont_delete_these_bones: frozenset[str] = frozenset({
...
dont_delete_these_main_bones: frozenset[str] = frozenset({
    "Hips", "Spine", "Chest", "Upper Chest", "Neck", "Head",
    "Left leg", "Left knee", "Left ankle", "Left toe",
    "Right leg", "Right knee", "Right ankle", "Right toe",
    "Left shoulder", "Left arm", "Left elbow", "Left wrist",
...
bone_reweight_to_parent: tuple[str, ...] = (
    "ShoulderC_\\L",
...
def _make_conflicting_bones():
    # TODO: This is a messy structure, can it be improved?
    base_bone_list_conflicting_names: tuple[tuple[list[str], str, str], ...] = (
    ...
    # If we need names that can have multiple "\\L" or "\\Left" in them, then we'll need to allow replacements to
    # replace more than one occurence.
    def replace_left(s: str):
        if "\\L" in s:
            s = s.replace('\\Left', 'Left', 1).replace('\\L', 'L', 1)
        return s

    def replace_right(s: str):
        if "\\L" in s:
            s = s.replace('\\Left', 'Right', 1).replace('\\L', 'R', 1)
        return s

    conflicting_bones = []
    for name_list, input_name1, input_name2 in base_bone_list_conflicting_names:
        if '\\L' not in input_name1:
            conflicting_bones.append((name_list, input_name1, input_name2))
            continue

        names0_l = list(map(replace_left, name_list))
        name1_l = input_name1.replace("\\Left", "Left", 1).replace("\\L", "L", 1)
        name1_r = input_name1.replace("\\Left", "Right", 1).replace("\\L", "R", 1)
        name2_l = ""
        name2_r = ""
        if "\\L" in input_name2:
            # FIXME?: Bug that name2 can be left as ""?
            name2_l = input_name2.replace("\\Left", "Left", 1).replace("\\L", "L", 1)
            name2_r = input_name2.replace("\\Left", "Right", 1).replace("\\L", "R", 1)
        conflicting_bones.append((names0_l, name1_l, name2_l))

        # FIXME: Previous code continued to append to names0, this does the same behaviour, but it is probably a bug.
        names0_r = names0_l + list(map(replace_right, name_list))
        conflicting_bones.append((names0_r, name1_r, name2_r))

    return conflicting_bones


bone_list_conflicting_names = tuple(_make_conflicting_bones())
del _make_conflicting_bones

bone_finger_list: tuple[str, ...] = (
    "Thumb0_",
    "IndexFinger1_",
    "MiddleFinger1_",
    "RingFinger1_",
...
#   L/R = \\L
################################
_bone_rename: dict[str, list[str]] = {
    "Hips": [
        "LowerBody",
        "Lower_Body",
        "Mixamorig_Hips",
    ...
    "Spine": [  # This is a list of all the spine and chest bones. They will be correctly fixed
        "Spine",  # First entry!
...
bone_reweight: dict[str, list[str]] = {
    ...
    "Left arm": [  # This has apparently no side in the name
        "エプロンArm",
    ],
    "\\Left elbow": [
        "Elbow1_\\L",
...
_bone_rename_fingers: dict[str, list[str]] = {
    "Thumb0_\\L": [
        # "Arm_\\Left_Finger_1a",
        "T_1_\\L",
        "\\LThumb1",
